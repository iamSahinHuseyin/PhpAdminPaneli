<?php
//Veritabanı oturumunu başlatmak.
@session_start();
@ob_start();
define("DATA","data/");
...

		{
			...
			if(!empty($_POST["kullanici"]) && !empty($_POST["sifre"]))
			{
				...
				if($kontrol!=false)//
				{
					...
					//Giriş yapıldıktan sonra anasayfaya yönlendirme işlemi.
					?>
										<meta http-equiv="refresh" content="0;url=<?=SITE?>" />
										<?php
					exit();//Oturumu sonlandırma işlemi.
				}
				else//Kullanıcı adı ve şifre hatalı ise tekrar login sayfasına yönlendirme işlemi
				{
					echo '<div class="alert alert-danger">Kullanıcı adı veya şifre hatalıdır.</div>';
				}
			}
			else//Boş alan bıraktıysak
			{
		...
		?>
			<form action="#" method="post">
				<div class="input-group mb-3">
					<input type="text" class="form-control" name="kullanici" placeholder="Kullanıcı Adınız" autocomplete="off">
