<?php
//Tabloda değer var ise.
if(!empty($_GET["tablo"]))
{
  //Tasbloyu gelen tablo ismini almak.
	$tablo=$VT->filter($_GET["tablo"]);
  //Veri getir fonksiyonumuzu çağırma.
  //Sql sorgumuza 2 şart verdik.
	$kontrol=$VT->VeriGetir("moduller","WHERE tablo=? AND durum=?",array($tablo,1),"ORDER BY ID ASC",1);
  //Kontrol değişkenimiz başarılı ise
	if($kontrol!=false)
	{
?>
       ...
       <div class="card">
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                <?php
                //Veri getir fonksiyonumuzu çağırma işlemi.
				$veriler=$VT->VeriGetir($kontrol[0]["tablo"],"","","ORDER BY ID ASC");
        //Veriler değişkeninde sonuç başarı bir şekilde döndüyse:
				if($veriler!=false)
				{
					$sira=0;
